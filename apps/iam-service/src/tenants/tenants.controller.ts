...
export class TenantsController {
  ...
  @Get()
  @Roles('SUPER_ADMIN')
  @ApiOperation({ summary: 'List all hospital tenants with stats' })
  ...
  @Patch(':id')
  @Roles('SUPER_ADMIN')
  @ApiOperation({ summary: 'Update hospital tenant details' })
  ...
  @Roles('SUPER_ADMIN')
  @ApiOperation({ summary: 'Reset hospital admin password — returns temporary credentials' })
  ...
  @ApiOperation({ summary: 'Update hospital/pharmacy profile (for printing — prescriptions, receipts, bills)' })
  updateProfile(@Param('id') id: string, @Body() dto: Partial<CreateTenantDto>) {
    return this.tenantsService.updateProfile(id, dto);
  }

  @Patch(':id/deactivate')
  @Roles('SUPER_ADMIN')
  @ApiOperation({ summary: 'Deactivate a hospital tenant' })
