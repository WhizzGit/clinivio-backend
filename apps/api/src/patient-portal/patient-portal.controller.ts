...
export class PatientPortalController {
  ...
  getAppointments(
    ...
    return this.svc.getAppointments(req.user.patientId, req.user.tenantId, page, limit);
  }
  ...
  cancelAppointment(@Param('id') id: string, @Request() req: any) {
    return this.svc.cancelAppointment(id, req.user.patientId, req.user.tenantId);
  }
  ...
  getConsultations(
    ...
    return this.svc.getConsultations(req.user.patientId, req.user.tenantId, page, limit);
  }
  ...
  getLabResults(
    ...
    return this.svc.getLabResults(req.user.patientId, req.user.tenantId, page, limit);
  }
  ...
  getInvoices(
    ...
    return this.svc.getInvoices(req.user.patientId, req.user.tenantId, page, limit);
  }

  // ── Discovery (public — for booking flow) ─────────────────────────────────────

  @Get('public/doctors')
  @ApiOperation({ summary: 'List available doctors (requires X-Tenant-Slug header)' })
