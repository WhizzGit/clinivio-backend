...
import { Room, Bed, IPDAdmission, BedStatus, IPDAdmissionStatus } from '@mediflow/database';
...
export class RoomsService {
  ...
  async update(id: string, tenantId: string, dto: UpdateRoomDto) {
    ...
    await this.roomRepo.update(id, updates);
  ...
  async updateBedStatus(id: string, tenantId: string, status: BedStatus, notes?: string) {
    ...
    await this.bedRepo.update(id, { status, notes: notes ?? undefined });
    return this.bedRepo.findOne({ where: { id }, relations: ['room'] });
  }

  async addBeds(roomId: string, tenantId: string, count: number) {
    const room = await this.findById(roomId, tenantId);

    const existingBeds = await this.bedRepo.find({ where: { roomId } });
    ...
    await this.bedRepo.save(newBeds);
    await this.roomRepo.update(roomId, { totalBeds: room.totalBeds + count });

    return this.findById(roomId, tenantId);
  }

  async getOccupancySummary(tenantId: string) {
    const stats = await this.bedRepo
      .createQueryBuilder('bed')
